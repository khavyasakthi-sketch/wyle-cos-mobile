...
// Reads obligations from Zustand store — brain dump tasks appear here automatically
...
function AddModal({ visible, onClose, onAdd }: any) {
  ...
  const handleAdd = () => {
    ...
    onAdd({
      ...
      emoji:         selType.emoji,
      title:         title.trim(),
      type:          selType.label.toLowerCase().replace(' ', '_'),
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={add.riskRow}>
                {(['low', 'medium', 'high'] as Risk[]).map(r => {
                  ...
                  const active = risk === r;
                  return (
                    <TouchableOpacity key={r} style={[add.riskBtn, active && { backgroundColor: rc, borderColor: rc }]} onPress={() => setRisk(r)}>
...
export default function ObligationsScreen({ navigation }: { navigation: NavProp }) {
  ...
  // ── Read from store (brain dump tasks appear here automatically) ────────────
            ...
            return (
              ...
              >
                <Text style={[styles.filterText, active && { color: f === 'medium' ? C.bg : C.white }]}>
                  {f === 'all' ? `All (${obligations.filter(o=>o.status==='active').length})` : f.charAt(0).toUpperCase() + f.slice(1)}
                </Text>
              </TouchableOpacity>
            );
          })}
      ...
      <ScrollView contentContainerStyle={styles.list} showsVerticalScrollIndicator={false}>
        ...
        {filtered.length === 0 && (
          <View style={styles.emptyState}>
            ...
            <Text style={styles.emptySub}>Use 🧠 brain dump to add tasks by voice</Text>
          </View>
        )}
...
const styles = StyleSheet.create({
  ...
  brainDumpBtn:{ width: 36, height: 36, borderRadius: 10, backgroundColor: `${C.verdigris}15`, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: `${C.verdigris}30` },
